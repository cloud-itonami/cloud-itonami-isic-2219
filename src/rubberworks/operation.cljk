(ns rubberworks.operation
  "OperationActor -- one rubber-products-manufacturer operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Rubber Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Compression-Seal Governor
  ...
  One graph run = one rubber-products-manufacturer operation (intake ->
  ...
  (:require [langgraph.graph :as g]
            ...
            [rubberworks.rubberadvisor :as rubberadvisor]
            [rubberworks.governor :as governor]
...
(defn build
  ...
  `rubberworks.store/Store`).
  opts:
    :advisor      -- a `rubberworks.rubberadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (rubberadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Rubber Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (rubberadvisor/-advise advisor store request)]
            {:proposal p :audit [(rubberadvisor/trace request p)]})))

      ;; Compression-Seal Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
